...
if (python code):
  put it here
```
# $$\hskip-0.5mm\boldsymbol{\hat{\hskip1mm\theta}}$$
...
I just learned about order statistics. Basicially, you sample from a distribution $$Y$$ and then order the sample $$y_i$$'s in ascending order. We can build a new distribution for the probabilities of the $$Y$$\`$$_{n}$$ ordered number. If you want the distribution of the smallest value in a sample of size $$n$$, it would be called $$Y$$\`$$_{min}$$
