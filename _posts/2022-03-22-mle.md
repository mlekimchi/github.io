...
We can build a new distribution for the probabilities of the $$Y \text{ \'} _{i}$$th ordered number. If you want the distribution of the smallest value in a sample of size $$n$$, it would be called $$Y$$\`$$_{min}$$. The second smallest would be $$Y$$\`$$_{2}$$,...the distribution of the largest value in a sample $$n$$ would be $$Y$$\`$$_{n}$$.
